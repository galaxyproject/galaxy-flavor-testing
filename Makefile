...
test_bioblend:
	...
	sudo -E su $(GALAXY_TRAVIS_USER) -c "export BIOBLEND_GALAXY_API_KEY=admin && export BIOBLEND_GALAXY_URL=http://localhost:8080 && export PATH=$(GALAXY_HOME)/.local/bin/:$(PATH) && cd $(GALAXY_HOME)/bioblend-master && tox -e $(TOX_ENV) -- -e 'test_download_dataset' -e 'test_tool_dependency_install' -e 'test_download_history'"
